...
export const waterQualityLogs = sqliteTable("water_quality_logs", {
    ...
    isViolation: integer("is_violation", { mode: "boolean" }).default(false),
});

export const wasteManifests = sqliteTable("waste_manifests", {
    id: text("id").primaryKey(),
    ...
    generatorDate: integer("generator_date", { mode: "timestamp" }).notNull(),
...
export const simpelRecords = sqliteTable("simpel_records", {
    id: text("id").primaryKey(),
...
export const rkabSubmissions = sqliteTable("rkab_submissions", {
    id: text("id").primaryKey(),
    reportId: text("report_id").notNull().references(() => complianceReports.id, { onDelete: 'cascade' }),
    year: integer("year").notNull(),
    productionTarget: real("production_target"), // e.g. tons
    salesTarget: real("sales_target"),
    explorationCost: real("exploration_cost"),
    environmentalBudget: real("environmental_budget"),
    technicalDocLink: text("technical_doc_link"),
});

export const complianceComments = sqliteTable("compliance_comments", {
    id: text("id").primaryKey(),
...
export const ghgEmissions = sqliteTable("ghg_emissions", {
    id: text("id").primaryKey(),
    ...
    value: real("value").notNull(), // e.g., 500 (liters of fuel, kWh)
    unit: text("unit").notNull(), // e.g., Liters, kWh, kg
    emissionFactor: real("emission_factor").notNull(), // e.g., 2.68 (kg CO2/Liter)
    co2e: real("co2e").notNull(), // Calculated total (tCO2e)
    notes: text("notes"),
    evidenceUrl: text("evidence_url"),
});

export const amdalMilestones = sqliteTable("amdal_milestones", {
    id: text("id").primaryKey(),
    title: text("title").notNull(),
...
export const amdalRequirements = sqliteTable("amdal_requirements", {
    id: text("id").primaryKey(),
    ...
    lastMonitoredDate: integer("last_monitored_date", { mode: "timestamp" }),
...
export const isoAudits = sqliteTable("iso_audits", {
    id: text("id").primaryKey(),
...
export const properAssessments = sqliteTable("proper_assessments", {
    id: text("id").primaryKey(),
    year: integer("year").notNull(),
...
export const properInventory = sqliteTable("proper_inventory", {
    id: text("id").primaryKey(),
    assessmentId: text("assessment_id").notNull().references(() => properAssessments.id, { onDelete: 'cascade' }),
    waterQualityScore: real("water_quality_score").default(0),
    airQualityScore: real("air_quality_score").default(0),
    hazardousWasteScore: real("hazardous_waste_score").default(0),
    landQualityScore: real("land_quality_score").default(0),
    complianceLevel: text("compliance_level").$type<"Full" | "Partial" | "None">().default("None"),
});

export const properBeyondCompliance = sqliteTable("proper_beyond_compliance", {
    id: text("id").primaryKey(),
    assessmentId: text("assessment_id").notNull().references(() => properAssessments.id, { onDelete: 'cascade' }),
    energyEfficiencyScore: real("energy_efficiency_score").default(0),
    waterConservationScore: real("water_conservation_score").default(0),
    biodiversityScore: real("biodiversity_score").default(0),
    emissionReductionScore: real("emission_reduction_score").default(0),
    socialInnovationScore: real("social_innovation_score").default(0),
});
...
export const popalProfiles = sqliteTable("popal_profiles", {
    id: text("id").primaryKey(),
    name: text("name").notNull(),
    certificationNumber: text("certification_number").notNull(),
    certifiedBy: text("certified_by").notNull().default("BNSP"), // e.g., BNSP
    validityStart: integer("validity_start", { mode: "timestamp" }).notNull(),
    validityEnd: integer("validity_end", { mode: "timestamp" }).notNull(),
    status: text("status").$type<"active" | "expired" | "revoked">().default("active"),
    companyId: text("company_id").default("DEFAULT_COMPANY"),
    profileImageUrl: text("profile_image_url"),
});

export const wastewaterLogs = sqliteTable("wastewater_logs", {
    id: text("id").primaryKey(),
    logDate: integer("log_date", { mode: "timestamp" }).notNull(),
    phLevel: real("ph_level").notNull(), // Typical BMAL: 6-9
    codLevel: real("cod_level").notNull(), // Dependent on industry (e.g. 100 mg/L)
    bodLevel: real("bod_level").notNull(), // Dependent on industry (e.g. 50 mg/L)
    tssLevel: real("tss_level").notNull(), // Dependent on industry (e.g. 50 mg/L)
    ammoniaLevel: real("ammonia_level").notNull(),
    debitOutfall: real("debit_outfall").notNull(), // m3/day
    popalId: text("popal_id").references(() => popalProfiles.id, { onDelete: 'set null' }),
    notes: text("notes"),
    isViolation: integer("is_violation", { mode: 'boolean' }).default(false), // Auto-flagged by system
});

// --- ISO 14001:2015 SUPER GOD-TIER (PHASE 6) ---

export const isoContext = sqliteTable("iso_context", {
    id: text("id").primaryKey(),
...
export const isoAspects = sqliteTable("iso_aspects", {
    id: text("id").primaryKey(),
    ...
    isSignificant: integer("is_significant", { mode: "boolean" }).notNull(),
...
export const isoLegalRegistry = sqliteTable("iso_legal_registry", {
    id: text("id").primaryKey(),
...
export const isoObjectives = sqliteTable("iso_objectives", {
    id: text("id").primaryKey(),
...
export const isoRisks = sqliteTable("iso_risks", {
    id: text("id").primaryKey(),
...
export const isoCAPA = sqliteTable("iso_capa", {
    id: text("id").primaryKey(),
...
export const isoInterestedParties = sqliteTable("iso_interested_parties", {
    id: text("id").primaryKey(),
    party: text("party").notNull(), // e.g. Government, Local Community, Employees
    needs: text("needs"), // What they expect
    expectations: text("expectations"),
    isLegalObligation: integer("is_legal_obligation", { mode: "boolean" }).default(false),
    status: text("status").$type<"Active" | "Archived">().default("Active"),
    updatedAt: integer("updated_at", { mode: "timestamp" }),
});

// Clause 8.1: Operational Controls (OCP)
export const isoOperationalControls = sqliteTable("iso_operational_controls", {
    id: text("id").primaryKey(),
...
export const isoEmergencyPrep = sqliteTable("iso_emergency_prep", {
    id: text("id").primaryKey(),
    ...
    lastDrillDate: integer("last_drill_date", { mode: "timestamp" }),
    nextDrillDate: integer("next_drill_date", { mode: "timestamp" }),
...
export const isoPerformanceMonitoring = sqliteTable("iso_performance_monitoring", {
    id: text("id").primaryKey(),
...
export const isoInternalAudits = sqliteTable("iso_internal_audits", {
    id: text("id").primaryKey(),
...
export const isoManagementReviews = sqliteTable("iso_management_reviews", {
    id: text("id").primaryKey(),
...
export const properCriteriaResults = sqliteTable("proper_criteria_results", {
    id: text("id").primaryKey(),
    assessmentId: text("assessment_id").notNull().references(() => properAssessments.id, { onDelete: 'cascade' }),
...
export const properCommunityDev = sqliteTable("proper_community_dev", {
    id: text("id").primaryKey(),
    assessmentId: text("assessment_id").notNull().references(() => properAssessments.id, { onDelete: 'cascade' }),
    programName: text("program_name").notNull(),
    budget: real("budget").default(0),
    beneficiaries: integer("beneficiaries").default(0),
    sroiScore: real("sroi_score"), // Social Return on Investment
...
export const domesticWasteLogs = sqliteTable("domestic_waste_logs", {
    id: text("id").primaryKey(),
    date: integer("date", { mode: "timestamp" }).notNull(),
    ...
    weight: real("weight"), // Optional if using volume
    volume: real("volume"), // Optional if using weight
...
export const wastePartners = sqliteTable("waste_partners", {
    id: text("id").primaryKey(),
    name: text("name").notNull(),
...
export const wasteSources = sqliteTable("waste_sources", {
    id: text("id").primaryKey(),
    name: text("name").notNull(),
    description: text("description"),
...
export const esgTopics = sqliteTable("esg_topics", {
...
export const esgDisclosures = sqliteTable("esg_disclosures", {
...
export const esgQuestions = sqliteTable("esg_questions", {
    id: text("id").primaryKey(),
...
export const esgAnswers = sqliteTable("esg_answers", {
    id: text("id").primaryKey(),
...
export const esgAssessments = sqliteTable("esg_assessments", {
    id: text("id").primaryKey(),
    year: integer("year").notNull(),
    title: text("title"),
